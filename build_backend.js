...
const GENRES = {
  fantasy: ['Dragon', 'Wizard', 'Castle', 'Potion'],
  scifi: ['Spaceship', 'Alien', 'Laser', 'Planet'],
...
function createRoom(roomId, config) {
  const room = {
    id: roomId,
    state: 'LOBBY_OPEN',
    config: config || { imposters: 1, rounds: 1, genre: 'locations' },
    players: new Map(),
    currentRound: 1,
    turn: null,
    order: [],
...
function assignRoles(room) {
  ...
  const pool = GENRES[room.config.genre] || GENRES.locations;
...
function calculateScoring(room) {
  ...
  room.impostersList.forEach(impId => {
    ...
    if (C === N && N > 0) {
      ...
      impPlayer.score += maxBonus;
    } else {
      impPlayer.score += Math.floor(maxBonus / 2);
    }
  });
  ...
  activeCrewIds.forEach(crewId => {
    ...
    if (isCorrect) room.players.get(crewId).score += baseCorrectScore;
    ...
    if (round >= 2 && v.confidence) {
      ...
      if (isCorrect) room.players.get(crewId).score += betScore;
      else room.players.get(crewId).score -= betScore;
    }
  });
}
...
io.on('connection', (socket) => {
  ...
  socket.on('join_room', (roomId, playerName) => {
    ...
    room.players.set(playerId, { name: playerName, socketId: socket.id, active: true, role: null, word: null, score: 0 });
    socket.roomId = roomId; socket.playerId = playerId;
    
    socket.join(roomId);
    socket.emit('join_success', { token, playerId });
    io.to(roomId).emit('lobby_update', Array.from(room.players.values()).map(p => ({ id: p.playerId, name: p.name, active: p.active })));
  });
  ...
  socket.on('force_start', (roomId) => {
    ...
    setTimeout(() => {
      ...
      room.order = Array.from(room.players.keys()).sort(() => Math.random() - 0.5);
      startNextTurn(room);
    }, 10000);
  });
  ...
  socket.on('disconnect', () => {
    if (socket.roomId && socket.playerId) {
      ...
      if (room) {
        const player = room.players.get(socket.playerId);
        if (player && player.socketId === socket.id) player.active = false;
      }
    }
  });
});
...
function endVoting(room) {
  ...
  if (room.currentRound >= room.config.rounds) {
    ...
    room.leaderboard = Array.from(room.players.entries()).map(([id, p]) => ({
      id, name: p.name, role: p.role, score: p.score
